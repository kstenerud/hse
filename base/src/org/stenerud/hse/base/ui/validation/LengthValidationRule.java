...
public class LengthValidationRule implements ValidationRule
{
	 ...
	 * @param name the name of this rule.
	 * @param getter the getter that will get the value.
	 * @param minLength the minimum string length allowed (-1 = minLength is
	 *           same as maxLength).
	...
	public void validate(Messages messages)
	{
		...
		if ( length > 0 )
		{
			...
			if ( length < minLength )
			{
				throw new ValidationException(messages.get("validation.mustBe.atLeastLength", messages.get(name), String
						.valueOf(minLength)));
			}
			if ( length > maxLength )
			{
