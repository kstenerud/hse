...
import org.stenerud.hse.base.ui.validation.ConditionalRule;
import org.stenerud.hse.base.ui.validation.EqualToValidationRule;
import org.stenerud.hse.base.ui.validation.IntegerValidationRule;
import org.stenerud.hse.base.ui.validation.LengthValidationRule;
import org.stenerud.hse.base.ui.validation.NotEmptyValidationRule;
import org.stenerud.hse.base.ui.validation.NotEqualToValidationRule;
import org.stenerud.hse.base.ui.validation.NotOneOfValidationRule;
import org.stenerud.hse.base.ui.validation.OneOfValidationRule;
import org.stenerud.hse.base.ui.validation.RegexValidationRule;
import org.stenerud.hse.base.ui.validation.StrippingValueGetter;
import org.stenerud.hse.base.ui.validation.TextDateValidationRule;
...
public class ValidationRuleMaker
{
	 ...
	 * If a component-associated rule throws a ValidationException, the
	 * associated component's focus is set.
	 * 
	 * @param rule the rule.
	 ...
	 * Conditional rules are only checked if the conditional getter returns
	 * "true".
	 * 
	 * @param rule the rule.
	...
	/**
	 * Add a rule, conditional on a CheckBox. <br>
	 * The rule will fire if the checkbox is checked.
	 * 
	 * @param rule the rule.
	 * @param conditionalField the checkbox.
	 * @param invert if false, invert the test on the checkbox (fire if false).
	 */
	public void addConditionalRule(ValidationRule rule, CheckBox conditionalField, boolean invert)
	{
		addRule(makeConditionalRule(rule, new CheckBoxConditionalGetter(conditionalField, invert)));
	}

	/**
	 * Add a rule, conditional on a SelectFieldHelper. <br>
	 * The rule will fire if the specified value is selected.
	 * 
	 * @param rule the rule.
	 * @param conditionalField the field helper.
	 * @param trueValue the value that results in true.
	 * @param invert if false, invert the test (fire if the specified value
	 *           isn't selected).
	 */
	public void addConditionalRule(ValidationRule rule, SelectFieldHelper conditionalField, String trueValue,
			boolean invert)
	{
		addRule(makeConditionalRule(rule, new SelectFieldHelperConditionalGetter(conditionalField, trueValue, invert)));
	}
	 ...
	 * @param invert if false, invert the test on the text field (fire if the
	 *           field doesn't contain the specified value).
	 */
	 ...
	 * @param minLength the minimum length allowed (-1 means minLength is same
	 *           as maxLength).
	 ...
	 * @param prePad if true, padding is on the front. Else padding is on the
	 *           back.
	 */
	 ...
	 * The rule will fail if the value is empty, contains only spaces, or
	 * contains only 0s.
	 * 
	 * @param name the rule name.
	 * @param field the field to check.
	...
	/**
	 * Add a "not one of" rule. The value must NOT match one of the invalid
	 * values.
	 * 
	 * @param name the rule name.
	 * @param field the field to check.
	...
	/**
	 * Add a regular expression rule. The value must match with the regular
	 * expression.
	 * 
	 * @param name the rule name.
	 * @param field the field to check.
	...
	{
		return makeComponentRule(new OneOfValidationRule(name, makeValueGetter(field), validValues, caseSensitive), field);
	}
	...
	/**
	 * Make a "not one of" rule. The value must NOT match one of the invalid
	 * values.
	 * 
	 * @param name the rule name.
	 * @param field the field to check.
	...
	{
		return makeComponentRule(new NotOneOfValidationRule(name, makeValueGetter(field), invalidValues, caseSensitive),
				field);
	}
	 ...
	 * @return the rule.
	 */
	public ValidationRule makeNotOneOfRule(String name, SelectFieldHelper field, Set invalidValues, boolean caseSensitive)
	{
		return makeComponentRule(new NotOneOfValidationRule(name, makeValueGetter(field), invalidValues, caseSensitive),
	...
	{
		return makeComponentRule(new NotEqualToValidationRule(name, makeValueGetter(field), value, caseSensitive), field);
	}
	...
	/**
	 * Make a regular expression rule. The value must match with the regular
	 * expression.
	 * 
	 * @param name the rule name.
	 * @param field the field to check.
